...
const Navbar = React.memo(
          ...
          >
            ...
            >
              ...
              <ul className='list-none hidden md:flex flex-row gap-4 ml-4 lg:gap-6 xl:gap-10'>
                {t('navBar.links', { returnObjects: true }).map(
                  (link, index) => (
                    <li
                      key={link.id}
                      className={`
  ...
  (prevProps, nextProps) => {
      // Implement custom comparison logic if needed
      // Return true if nextProps will result in the same rendered output as prevProps, otherwise return false
      return (
        prevProps.animationClass === nextProps.animationClass &&
        prevProps.selectedLang === nextProps.selectedLang &&
        prevProps.activeMediaQuery === nextProps.activeMediaQuery
        // Consider other props as necessary
      );
    }
);
